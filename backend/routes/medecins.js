...
// ✅ GET liste des médecins — accessible aussi aux patients (pour le select RDV)
router.get('/medecins', protect, roles('admin', 'secretaire', 'medecin', 'patient'), async (req, res) => {
  try {
    const medecins = await User.find({ role: 'medecin' })
      .select('nom prenom specialite')   // ✅ minimal, pas de données sensibles
      .sort({ nom: 1 });
...
router.get('/:id', protect, roles('admin', 'secretaire', 'medecin'), async (req, res) => {
  try {
...
router.put('/:id', protect, roles('admin'), async (req, res) => {
  try {
    const body = { ...req.body };
    delete body.mot_de_passe; // ✅ ne jamais modifier le mot de passe via cette route
